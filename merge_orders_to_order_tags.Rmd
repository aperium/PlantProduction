...
mtag_orders <- foreach::foreach(i=order(mtag_files), .combine = \(x,y) dplyr::rows_upsert(x,y,by = c("MT Sales Order #", "MT ID")), .packages = c("foreach")) %dopar% { 
  foreach(j=c(2,1), .combine = dplyr::left_join) %dopar% {
    ...
    dplyr::mutate(vendor = "MasterTag",
                  ...
                  "MT Item Description" = `MT Item Description` |> str_remove_all("[:punct:]|®|™") |> str_squish())
...
```{r filter}
## TODO  this is super slow. Might be a good thing to try multidplyr
## https://cran.r-project.org/web/packages/multidplyr/vignettes/multidplyr.html#:~:text=multidplyr%20is%20a%20backend%20for,transparently%20support%20multi%2Dprocess%20parallelism.
...
p_loc <- apply(scores, 2, which.max)
# plant_names[p_loc]

t_loc <- apply(scores, 1, which.max)
# tag_names[t_loc]


plant_boolmat <- big.matrix(nrow = length(plant_names), ncol = length(tag_names))
tag_boolmat <- big.matrix(nrow = length(plant_names), ncol = length(tag_names))
foreach(i=1:length(plant_names), .combine = \(...){NULL}) %do% {
  foreach(j=1:length(tag_names), .combine = \(...){NULL}) %do% {
    plant_boolmat[p_loc[j],j] <- plant_names[p_loc[j]] == plant_names[i]
    tag_boolmat[i,t_loc[i]] <- tag_names[t_loc[i]] == tag_names[j]
  }
}
