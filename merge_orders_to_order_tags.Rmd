...
- [ ] subtract tags ordered from tags needed
  ...
  - [ ] join to plant orders table with some name alignemtn function, probably using fuzzyjoin. Consider Biostrings::pairwiseAlignment()
  - [ ] subtract from need_to_order
- [ ] combine multiple history files from ball so I don't have to download the whole thing every time. use dplyr::upsert()
...
if (("reticulate" %in% loaded_pacs) & !reticulate::py_available(initialize = TRUE)) install_python()
...
b_files <- dir_path |> fs::path("Ball export") |> fs::dir_ls() |> fs::path_filter(regex = "order_download_.*\\.xlsx")
registerDoParallel(cores=n_cores)
...
mtag_dir <- "Greenstreet Growers/TeamSite - Documents/Shared/Production Greenstreet/Production Planning QC/Tags/MasterTag" 
