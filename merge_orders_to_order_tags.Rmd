...
name_match <- function(x,y) {
  ...
  doParallel::registerDoParallel(cores=n_cores*3/4)
  scores <- foreach::foreach(i=1:length(y_u), .combine = rbind) %dopar% {
    print("1a")
...
aligned <- fuzzyjoin::fuzzy_full_join(filtered, mtag_orders, by=c("Description"="MT Item Description"), match_fun = name_match)
