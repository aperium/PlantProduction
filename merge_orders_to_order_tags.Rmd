...
cran_pacs <- c("rlang", "pkgbuild","librarian", "pak", "rmarkdown", "tidyselect", "tidyverse", "magrittr", "stats", "measurements", "knitr", "foreach", "doParallel", "parallel", "fs", "janitor", "stringr", "readxl", "writexl","fuzzyjoin", "chromote", "pdftools", "lubridate", "readr", "tidyr", "forcats", "purrr", "stringdist", "bigmemory", "multidplyr") |> unique() # "rvest", "polite", "XML", "tabulizer", "gt"
...
name_match <- function(x,y) {
  ...
  xy_setdiff <- generics::setdiff(x_u, y_u)
  
  if(length(xy_setdiff) == 0) {
    out1 <- mapply(\(m,n) {str_equal(m,n,ignore_case = TRUE)},x,y)
    return(out1)
  }
  
  # x_u <- generics::setdiff(x_u,xy_intersect)
  # y_u <- generics::setdiff(y_u,xy_intersect)
  x_u <- generics::intersect(x_u,xy_setdiff)
  y_u <- generics::intersect(y_u,xy_setdiff)

  registerDoParallel(cores=n_cores*2/3)
  scores <- foreach(i=1:length(y_u), .combine = rbind) %dopar% {
    Biostrings::pairwiseAlignment(x_u, y_u[i],scoreOnly = TRUE)
    } |> unlist()
  stopImplicitCluster()
  ...
  x_y_index <- foreach(j=1:length(x_loc), .combine = rbind) %do% {
    c(xname = x_u[x_loc[j]], yname = y_u[j], xi = x_loc[j] |> as.integer(), yi = j |> as.integer())
  }

  y_x_index <- foreach(i=1:length(y_loc), .combine = rbind) %do% {
    c(yname = y_u[y_loc[i]], xname = x_u[i], yi = y_loc[i] |> as.integer(), xi = i |> as.integer())
  }
...
testout <- name_match(mtag_orders$`MT Item Description`, filtered$Description)

# Next TODO: compare the input lists to the lookup table to return boolian lists compatible with fuzzyjoin(match_fun)



# plant_boolmat |> as.integer() |> sum()
# tag_boolmat |> as.integer() |> sum()

# tag_boolmat <- matrix(nrow = length(plant_names), ncol = length(tag_names))
# foreach(i=1:length(plant_names), .combine = \(...){}) %do% {
#   foreach(j=1:length(tag_names), .combine = \(...){}) %do% {
#     tag_boolmat[i,j] <- tag_names[t_loc[i]] == tag_names[j]
#   }
# }
# tag_boolmat |> as.integer() |> sum()

# plant_boolmat <- foreach(i=1:length(plant_names), .combine = rbind) %do% {
#   foreach(j=1:length(tag_names)) %do% {
#    plant_names[p_loc[j]] == plant_names[i]
#   } |> unlist()
# } |> unlist()

# tag_boolmat <- foreach(i=1:length(plant_names), .combine = rbind) %do% {
#   foreach(j=1:length(tag_names)) %do% {
#     tag_names[t_loc[i]] == tag_names[j]
#   } |> unlist()
# } |> unlist()
# 
# both_boolmat <- plant_boolmat & tag_boolmat
# both_boolmat |> as.integer() |> sum()
# 
# tag_names[apply(both_boolmat, 1, which, useNames=FALSE) |> unlist()] |> cbind(plant_names)
# 
# plant_names[apply(both_boolmat, 2, which, useNames=FALSE) |> unlist()] |> cbind(tag_names)
# 
# 
# foreach(i=1:length(plant_names)) %do% tag_names[unlist(apply(both_boolmat, 1, which))[i]] |> unlist() |> cbind(plant_names)

# foreach(i=1:length(plant_names)) %do% tag_names[apply(scores, 1, which.max)[i]] |> unlist() |> cbind(plant_names)
