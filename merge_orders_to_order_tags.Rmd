...
# tag_names <- mtag_orders$`MT Item Description` |> unique()
# plant_names <- filtered$Description |> unique() |> extract2(24)
# # get(plant_names[1])
# dummy_align <- tag_names |> as_tibble_col("tagname") |>
#   add_column("score" = Biostrings::pairwiseAlignment(tag_names, plant_names,scoreOnly = TRUE) |> as.vector()) |>
...
string_alignment_prep <- function(x) {
  prep_one <- function(y) {
    y |> 
      str_to_lower() |> 
      str_extract_all("([:alpha:]{2,})|(([:digit:]*(\\.[:digit:]+)?)(?!c$))", simplify = TRUE) |> 
      str_squish() |> 
      unique() |> 
      as.character() |> 
      str_sort() |> 
...
# c("hello `this` 'is' another-sentence 3.5","but this isn't a very good one 288C") |> string_alignment_prep()
...
name_match <- function(x,y) {
  ...
  scores <- foreach::foreach(i=1:length(y_u), .combine = rbind) %dopar% {
    # print("1a")
    raw_score <- Biostrings::pairwiseAlignment(x_u, y_u[i],scoreOnly = TRUE)
...
aligned <- fuzzyjoin::fuzzy_full_join(filtered, tags_filtered, by=c("Description"="MT Item Description"), match_fun = name_match)
