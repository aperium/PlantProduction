...
plant_boolmat <- matrix(nrow = length(t_loc), ncol = length(p_loc))
tag_boolmat <- matrix(nrow = length(t_loc), ncol = length(p_loc))
foreach(i=1:length(p_loc), .combine = \(...){NULL}) %do% {
  foreach(j=1:length(t_loc), .combine = \(...){NULL}) %do% {
    plant_boolmat[p_loc[j],j] <- plant_names[p_loc[i]] == plant_names[j]
    tag_boolmat[i,t_loc[i]] <- tag_names[t_loc[j]] == tag_names[i]
  }
}
