...
plants_prep <- plants_filtered |>
  summarise(`QTY Ordered` = sum(`QTY Ordered`), .by = any_of(c("Description","Product #"))) |>
  rename_with(\(x) paste0("plants_",x))

tags_prep <- tags_filtered |>
  summarise(`Ordered Quantity` = sum(`Ordered Quantity`), .by = any_of(c("MT Item Description","MT ID"))) |>
