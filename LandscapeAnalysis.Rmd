...
select_cols_x <- c("Business date", "Presumed cost", "Cost", "Document", "Event number", "Document ID", "Item number", "Description", "Item type", "Price", "Quantity shipped", "Gross ext prc", "Sequence #") %>% 
  match(.,ord_hist_ln_names$value) %>% 
  foreach(i=.) %dopar% ord_hist_ln_names$key[i] %>% 
  unlist() |>
  c(match_cols_x, shared_fields$key) |>
  setdiff(c("BUS_DAT_EXT","DOC_ID_EXT")) |>
  str_unique()
...
select_cols_y <- c("Business date", "Document", "Event number", "Customer #", "Note") %>% 
  ...
  unlist() |> 
  c("SHIP_ADRS_1","SHIP_ADRS_2","SHIP_CITY","SHIP_STATE","SHIP_ZIP_COD","SHIP_CNTRY") |>
...
## debugging
tmp <- ord_hist_names |> filter(key %in% select_cols_y)
