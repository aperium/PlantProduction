...
registerDoParallel(cores=n_cores-1)
ord_hist_ln_data <- foreach::foreach(i=1:length(ord_hist_ln_file), .combine = rbind, .packages=c("tidyverse","doParallel")) %dopar% 
    (read_csv(ord_hist_ln_file[i], col_names = TRUE, guess_max = 10000, num_threads = n_cores-1, col_select=(c("Business date", "Presumed cost", "Cost", "Document", "Item number", "Description", "Item type", "Price", "Quantity shipped", "Gross ext prc", "Sequence #") %>% match(.,ord_hist_ln_names$value) %>% foreach(i=.) %dopar% ord_hist_ln_names$key[i] %>% unlist()))) %>%
