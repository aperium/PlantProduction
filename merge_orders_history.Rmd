...
b_orders <- foreach::foreach(i=order_files(b_files, baseName = "order_download_", parseFun = ymd_hms), .combine = \(x,y) dplyr::rows_upsert(x,y,by = c("Order Number", "Order Line"))) %dopar% { read_xlsx_flex(b_files[i], guess_max = as.integer(.Machine$integer.max/100)) |>
...
mtag_orders <- foreach::foreach(i=order_files(mtag_files, baseName = "GREENSTREET_GROWERS_INC_Order Export_", parseFun = mdy), .combine = \(x,y) dplyr::rows_upsert(x,y,by = c("MT Sales Order #", "MT ID")), .packages = c("foreach")) %dopar% { 
