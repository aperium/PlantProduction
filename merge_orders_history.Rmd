...
cran_pacs <- c("tidyverse","writexl","fs","stringr","parallel","multidplyr","foreach","doParallel","generics","withr","rlang","janitor","magrittr","purrr")
...
b_orders <- foreach::foreach(i=order_files(b_files, baseName = b_baseName, parseFun = ymd_hms), .combine = \(x,y) dplyr::rows_upsert(x,y,by = c("Order Number", "Order Line")), .packages = c("readxl")) %dopar% { 
  read_xlsx(b_files[i], guess_max = as.integer(.Machine$integer.max/100)) |>
    dplyr::mutate(vendor = "Ball",
                 ...
                 "Material Format" = string_alignment_alike()) |> ##TODO
