...
plants_joined <- full_join(e_combined, b_orders, by=alignment) |>
  ...
  mutate(type = case_when(
    ...
    Size |> str_detect(regex("((lin)|(split)|(plug)|(cust(pl)?)|(b?kit))([:space:]PW)?[:space:]+[:digit:]{2}[:digit:]?", ignore_case = T)) ~ "plug",
    Size |> str_detect(regex("(MPL)|([:digit:]+[KHM])", ignore_case = T)) ~ "seed",
    Size |> str_detect(regex("(URC)|(callus)|(unrooted)", ignore_case = T)) ~ "urc",
    Size |> str_detect(regex("(brt)|(bareroot)", ignore_case = T)) ~ "bareroot",
