...
## define paths
CP_path <- "../../CP export"
Ball_path <- "../../Ball export"
CH_path <- "../../CommerceHub export"

# CounterPoint Files
tkt_hist_ln_file <- CP_path %>% dir_ls(regexp = "PS_TKT_HIST_LIN_[[:digit:]]{4}.*\\.(c|C)(s|S)(v|V)")
tkt_hist_file <- file.path(CP_path, "PS_TKT_HIST.csv")
item_data_file <- file.path(CP_path, "IM_ITEM.csv")

# Ball Webtrack Files
ball_file <- Ball_path %>% file.path("order_download_2023100408085501.csv")
...
CP_prediction_weeks |>
  filter(Size == "IN4",
         `Sub Departments` == "HERB") |>
  mutate(quant_prop = Quantity / sum(Quantity))
