...
#
cran_pacs <- c("rlang", "pkgbuild","rmarkdown", "BiocManager", "tidyselect", "tidyverse", "magrittr", "stats", "measurements", "knitr", "foreach", "doParallel", "parallel", "fs", "janitor", "readxl", "writexl","fuzzyjoin", "plyr", "stringdist", "multidplyr") 
...
prediction_year <- 2025 #year(now())*1
...
# CounterPoint Files
tkt_hist_ln_file <- CP_path %>% fs::dir_ls(regexp = "PS_TKT_HIST_LIN_[[:digit:]]{4,8}.*\\.[cC][sS][vV]")
...
# output path
out_path <- "/Greenstreet Growers/TeamSite - Documents/Shared/Production Greenstreet/Production Planning QC/Plant Sales and Orders/" |> str_c(prediction_year) |> fs::path_home()
...
CP_data %>%
  ...
  write_xlsx_flex(str_c("2025","-herbveg-sales-pred-v1.xlsx"))
