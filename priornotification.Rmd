...
tracking_df |>
  dplyr::rows_patch(unite(data=tracking_df,col="zip",all_of(c("Customer Name","Comments","Receiving City","Receiving State")), sep = " ", remove = FALSE, na.rm = TRUE) |>
                      pull(var=zip) |> 
                      google_get_address() |> 
                      as_tibble()  |> 
                      separate_wider_delim(cols = value, delim = ", ", names = c("Receiving Street","Receiving City","statezip")) |>
                      separate_wider_delim(cols = statezip, delim = " ", names = c("Receiving State","Receiving ZIP")) |>
                      select(-"Receiving Street"))
