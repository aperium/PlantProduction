...
## TODO let query be a list of strings. Internally reduce to unique queries and then re expand to same lenth as query for return.
...
# c("greenstreet gardens braddock road alexandria VA","giant e glebe Alexandria VA","the white house washington DC", "gateway arch St louis MO") |>
# google_get_addresses()
...
orders_path <- file.path(paste0(ship_year,"w",ship_week),paste0("uninvoiced_",ship_year,"w",ship_week,".xlsx"))
tracking_path <- file.path(paste0(ship_year,"w",ship_week),paste0("tracking_",ship_year,"w",ship_week,".xlsx"))

states_path <- file.path("statestable.xlsx")
contact_path <- file.path("contact_info_greenstreet_growers.xlsx")
treatment_path <- file.path("treatment_log_2024.xlsx")
...
tracking_df <- tracking_path %>% read_xlsx_flex() %>%
  ...
  mutate("Receiving City" = `Receiving Location` %>% str_squish() %>% str_remove("[:punct:]?[:space:][:alpha:]{2}$"),
         ...
         "Receiving ZIP" = google_get_addresses(paste(`Customer Name`,Comments,`Receiving City`,`Receiving State`)) |> str_squish() |> str_extract("[:digit:]{5}(-[:digit:]{4})?$")) %>%
  select(-`Receiving Location`, -`Drop Point Location`, -`Drop Point Name`) |>
  distinct(`Supplier Description`,`Customer Name`, `Ship Week`, `Document No.`, .keep_all = TRUE) ## This is a work around for when there are lines for a single order shipping by multiple carriers. Improvment might make a more informed decision about which line to choose. This just picks the first one listed.  
...
## TODO stop if there are no prio notifications to send.
...
nearestPastOrPresent = function(x,y) {sapply(x,function(a) {max(y[y<=a]) |> as_date() |> interval(a) |> is_weakly_less_than(30*24*3600)}) }
...
joined_df_b <- joined_df_a |>
  ...
  fuzzyjoin::fuzzy_full_join(treatment_df, by=c("Ship Date" = "Treatment Date"), match_fun=nearestPastOrPresent)
...
sapply(joined_df_b$`Receiving State` |> unique(), function(a) {
  joined_df_b |>
    ...
    write_xlsx_flex(file.path(paste0(ship_year,"w",ship_week),paste0(a,"_prior_notifications_greenstreet_growers_",ship_year,"w",ship_week,".xlsx")))
})
